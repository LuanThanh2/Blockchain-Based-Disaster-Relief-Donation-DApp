...
const UnifiedTempleDonationPage = () => {
  ...
  const fetchActiveTempleAdmins = async () => {
    try {
      ...
      setTemples(Array.isArray(rows) ? rows : []);
    } catch (error) {
    ...
    try {
      ...
      if (receipt) {
        ...
        const payload = {
          ...
          txHash: tx.hash,
          gasPrice: effectiveGasPrice ? Number(effectiveGasPrice.toString()) : 0,
          transactionFee: totalCostWei ? Number(totalCostWei.toString()) : 0,
