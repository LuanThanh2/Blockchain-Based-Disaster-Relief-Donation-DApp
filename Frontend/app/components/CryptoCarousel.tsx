...
const CryptoCarousel: React.FC<CryptoCarouselProps> = ({ onSelectCrypto }) => {
  const [allCryptoPrices, setAllCryptoPrices] = useState<CarouselCoin[]>([]);
  ...
  useEffect(() => {
    const fetchCryptoPrices = async () => {
      try {
        ...
        let carouselCurrencies: CarouselCoin[] = marketData.map((coin: any): CarouselCoin => ({
          id: String(coin.id),
          name: String(coin.name),
          symbol: String(coin.symbol).toUpperCase(),
          price: Number(coin.current_price),
          image: String(coin.image),
          price_change_percentage_24h: typeof coin.price_change_percentage_24h === "number" ? coin.price_change_percentage_24h : undefined,
          market_cap: typeof coin.market_cap === "number" ? coin.market_cap : undefined,
        }));

        // Ensure MATIC is included
        const maticInCarousel = carouselCurrencies.find((coin) => coin.id === "matic-network");
        ...
        const uniqueCurrencies = Array.from(
          new Map<string, CarouselCoin>(carouselCurrencies.map((item) => [item.id, item])).values()
        ).slice(0, 18);

        setAllCryptoPrices(uniqueCurrencies);

        const totalMarketCap = uniqueCurrencies.reduce(
          (sum: number, crypto: CarouselCoin) => sum + (crypto.market_cap || 0),
          0
        );
